...
if (!empty($search)) {
    $searchCondition = "AND p.title LIKE ?";
    $searchParams = ["%$search%"];
}

// Получаем последние пасты с лайками и дислайками
$stmt = $pdo->prepare("
    ...
    WHERE p.visibility = 'public' $searchCondition
...
// Получаем общее количество паст для пагинации
$countStmt = $pdo->prepare("SELECT COUNT(*) FROM pastes WHERE visibility = 'public' $searchCondition");
...
<head>
    ...
    <title>Recent Pastes - example.com</title>
...
<body class="bg-background min-h-screen flex flex-col pt-12">
    ...
    <div class="container mx-auto flex justify-center">
        <div class="w-full max-w-[1200px] flex flex-col items-center">
            <h1 class="text-textColor font-extrabold mb-2 pagetitle" style="font-size: 24px; font-weight: 1000; font-weight: bold;">Recent Pastes</h1>
            ...
            <div class="w-full max-w-[600px] mb-6">
                <div class="flex gap-2">
                    <input type="text" 
                           ...
                           class="flex-1 bg-backgroundTextarea text-textColor px-4 py-2 rounded-lg border border-gray-600 focus:border-primary focus:outline-none">
                    <?php if (isset($_GET['search']) && !empty($_GET['search'])): ?>
                        <a href="recent.php" 
                           class="bg-gray-600 hover:bg-gray-500 text-white px-4 py-2 rounded-lg font-bold transition-colors">
            ...
            <div class="w-[1000px] max-w-full bg-[#1d1e3a] rounded-lg overflow-hidden pb-4 mb-6 shadow-sm" style="display:inline-block;">
                ...
                <?php if ($userRank['rank']): ?>
                    <strong>
                        <a href="user.php?id=<?php echo $pasteUser['id']; ?>" class="hover:underline font-bold" style="font-weight: bold !important;">
                            <?php
                                // Combine custom name color with rank class and keep style if present
                                $attrParts = [];
                                $classes = [];
    ...
    <a href="user.php?id=<?php echo htmlspecialchars($pasteUser['id']); ?>" class="hover:underline" style="font-weight: bold !important;">
        <?php
            // Build combined wrapper attributes (classes + style)
            $attrParts = [];
            $classes = [];
    ...
    <?php endforeach; ?>
            ...
            <!-- Pagination ABOVE content panel now -->
            <?php echo renderPagination($page, $totalPages, 'recent.php', !empty($search) ? ['search'=>$search] : []); ?>

            <!-- Green separator line -->
            <div class="w-full mx-auto my-4" style="border-bottom: 1px solid #00ff9d;"></div>

            <!-- Footer area remains below pagination -->
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (searchInput) {
                searchInput.addEventListener('input', function() {
                    ...
                    const separatorRow = document.querySelector('tbody tr td[colspan="5"]');
                    if (separatorRow) {
                        const separatorTr = separatorRow.parentElement;
                        const pinnedRows = document.querySelectorAll('tbody tr:not([style*="display: none"]) .fa-thumbtack');
                        if (pinnedRows.length === 0) {
                            separatorTr.style.display = 'none';
                        } else {
                            separatorTr.style.display = '';
                        }
                    }
                });
            }
